...
class DPProblems
  ...
  def make_change(amt, coins, coin_cache = {0 => 0})
    ...
    return 0.0 / 0.0 if amt < coins.first

    total = amt
    coins.each do |coin|
      change_count = make_change(amt - coin, coins, coin_cache)
      sub_result = 1 + next_change if next_change.is_a?(Integer)
      total = sub_result if result.nan? || sub_result < result
    end
